...
# Load Quran Passages from Training Data
...
# Return list of  Relevant Quran Passages with Score
...
# create Answer Extraction Template with few demonstrations
def template(query,retrieved_passages):
     # List of the most relevant examples (question,context,answer) for question
     results = predict_Question_rerank(query, model, search, all_passages)
     few_shot_examples = ""
     for r in results:
     ...
     prompt = few_shot_examples + f"Question: {query}\nContext: {retrieved_passages}\nAnswer:"
     return prompt
